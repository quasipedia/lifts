...
from enums import LiftStatus, PersonStatus, Commands, Messages
...
class Simulation:
    ...
    def _init_lifts(self):
        ...
        for description in self.description['lifts']:
            ...
            lift = Lift(self, description, LiftStatus.ready, floor)
    ...
    def _init_people(self):
        ...
        for pid in range(self.description['simulation']['population']):
            ...
            self.people.append(Person(self, pid_string, PersonStatus.busy,
                                      None, destination, event_time))

    def route(self, event, entity=None, **kwargs):
        '''Listen to events generated by the objects in the game.'''
        msg = 'Routing "%s --> %s"[payload: %s]'
        log.debug(msg, event, entity, kwargs)
        if event in Messages:
            self.interface.send_message(event, entity, **kwargs)
        elif event in Commands:
            try:
                self.lifts[entity].process(event, entity, **kwargs)
            except KeyError:
                msg = 'There is no lift called "{}"'.format(entity)
                self.route(Messages.error, msg=msg)
        else:
            log.error('Unknown event: "%s"'.format(event))
    ...
    def step(self):
        ...
        for command, entity, floor in self.interface.get_commands():
            self.route(command, entity, floor=floor)
